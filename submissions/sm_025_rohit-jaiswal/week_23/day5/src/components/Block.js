...
class Block extends React.Component{
    ...
    handleSubmit=(e)=>{
        ...
        this.setState({
            dataFaculty:[...this.state.dataFaculty,formData],
            dataEdt:[...this.state.dataEdt,formData],
            misc:true
        })
        let localData=[...this.state.dataFaculty,formData]
    ...
    removeFac=(id)=>{
        let userData = this.state.dataFaculty.filter(element=>element.id!==id)
        this.setState({
            dataEdt:[...userData]
        })
    ...
    sectionsSort=(e)=>{
        ...
        this.setState({dataEdt:this.state.dataFaculty.filter(element=>element.section===e.target.value)})
        )
    }
    ...
    subjectSort=(e)=>{
        (e.target.value==="all"?
        this.setState({dataEdt:this.state.dataFaculty}):
        this.setState({dataEdt:this.state.dataFaculty.filter(element=>element.subject===e.target.value)})
        )
    }

    render(){
        let enterFunction=(this.state.misc2)?this.handleUpdate:this.handleSubmit

        return(
            
            <div className="container-fluid">
                <h1 className="text-center text-success display-3 mb-5 font-weight-bold"> Welcome to Teacher's Management System</h1>
                <div className="col text-center justify-content-center">
                    <form onSubmit={enterFunction}>
                        <div className="form-group">
                            <label className="h5">Assign Teacher</label>
                            <input type="text" name="name" value={this.state.name} onChange={this.handleChange} className="form-control" placeholder="Teacher's Name"/>
                        </div>
                        <div className="form-group">
                ...
                {this.state.misc && <Dash data={this.state.dataEdt}
