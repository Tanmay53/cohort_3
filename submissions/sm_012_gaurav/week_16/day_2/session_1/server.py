...
def getAndPostData():
    if request.method == 'GET':
        try:
            ...
            usersPerPage = request.args.get('usersPerPage', default = 5, type = int)
            noOfPages = math.ceil(float(len(data)) / usersPerPage)
            if pageNo > noOfPages + 1:
                return json.dumps({'message': 'OOPS!! You are on the wrong link!!!', 'done': False})
            else:
                if pageNo == noOfPages + 1:
                    pageNo = 1
                lowerIndex = (pageNo - 1) * usersPerPage
                upperIndex = (pageNo) * usersPerPage
                return json.dumps({'data': data[lowerIndex : upperIndex], 'done': True, 'numberOfPages': noOfPages})
        except:
