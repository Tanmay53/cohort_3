...
def loggin():
    ...
    if request.method == 'POST':
        cursor = mysql.connection.cursor()
        cursor.execute(
            """SELECT salt FROM register where email=%s""", (
                request.json["email"],)
        )
        ...
        cursor = mysql.connection.cursor()
        cursor.execute(
            """SELECT password,id,type FROM register where password=%s""", (
                verify,)
        )
        ...
        if result:
            ...
            print(encoded_data)
            return {"token": str(encoded_data),"type":result[0]["type"]}
        else:
...
def register():
    ...
    if request.method == 'POST':
        ...
        cursor = mysql.connection.cursor()
        cursor.execute(
            """INSERT INTO register(id,name,email,phone,salt,password,type)values(NULL,%s,%s,%s,%s,%s,DEFAULT)""", (
                request.json["name"], request.json["email"], request.json["number"], salt, password)
        )
        cursor.connection.commit()
        cursor.close()
        return "success"
...
def name():
    ...
    if(ans):
        cursor = mysql.connection.cursor()
        cursor.execute(
            """SELECT name from register where id=%s""", (ans["id"],)
        )
        result = cursor.fetchall()
        cursor.connection.commit()
        cursor.close()
