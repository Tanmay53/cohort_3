...
function cartItems() {
  ...
  $("#totalCart").append(getArr1.length)
  for(let j=0; j<getArr1.length; j++){
    totalCart += getArr1[j].price
  }
  $("#cartVal").append("including all taxes RS:"+totalCart)
...
function topCards(){
  ...
  proData2.forEach((ele, i) => {
    $("#topCards").append(`
  <div class="col">
        <img src="${ele["proImage"]}" class="img-fluid" alt="icon">
          <div class="card-body">
            <h4 class="card-title">${ele["proName"]}</h4>
            <p class="card-text text-success ">${ele["proPrice"]}</p>
            <h5 href="#" class="">${ele["proBrand"]}</h5>
          </div>
  </div>
        `)
