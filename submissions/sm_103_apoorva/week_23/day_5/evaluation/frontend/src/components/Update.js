...
class Update extends Component {
    ...
    componentDidMount = () =>{
        ...
        if(JSON.parse(token) != null && JSON.parse(isLoggedIn) === true){
            ...
            axios.post('http://127.0.0.1:5000/getDetail',{
                "tid" : id
            },tokenCheck).then
            (res =>{
                console.log(res.data.teacher.cid)
                this.setState({
                    name:res.data.teacher.name,
                    email:res.data.teacher.email,
                    mobile:res.data.teacher.mobile,
                    subject:res.data.teacher.subject,
                    section:res.data.teacher.sid,
                    class:res.data.teacher.cid,
        ...
        return (
            <div>
                <div className="bgLogin mt-5">
                    ...
                    <div>
                        <button className="btn btn-primary mt-3 ml-3 mb-3" onClick={this.handleClick}>Update data</button>
                    </div>
                </div>
            </div>
        )
