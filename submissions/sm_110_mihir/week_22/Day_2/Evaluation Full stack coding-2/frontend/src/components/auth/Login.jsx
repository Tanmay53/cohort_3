  ...
  handleSubmit = async e => {
    ...
    if (result.payload.status === 200) {
      if (result.payload.data.error) alert(result.payload.data.message);
        ...
        console.log("success login.   token :" + result.payload.data.token);
        config = {
          method: "GET",
          url: "http://localhost:5000/user",
          headers: { Authorization: "Bearer " + result.payload.data.token }
        };
        const auth = await this.props.fetchAuthData(config);
        console.log(auth);
        if (auth.payload.status === 200 && !auth.payload.data.error) {
