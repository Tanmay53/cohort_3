       ...
       .then((myJson) => {
        //    console.log(myJson.items);
           myJson.items.forEach((ele) => {
               $('#tBody').append(`<tr><td>${ele.id}</td><td>${ele.login}</td><td>${ele.score}</td><td><button class="btn btn-primary getLink" value="${ele.repos_url}">Show Repos</button></td><td><img height="150px" width="120px" src="${ele.avatar_url}" alt="user's image"></img></td></tr>`);
           })
       })
    })
})
