...
function changePage(){
    ...
    createTableRow(students, (pageValue-1)*selectValue, pageValue*selectValue);  
...
function createTableRow(studentData, index1, index2){
    $('#userDataTable').empty();
        ...
        for(let i = index1; i < index2; i++){
            $('#userDataTable').append(`<tr><td> ${studentData[0][i].id}</td><td> ${studentData[0][i].first_name}</td><td> ${studentData[0][i].last_name}</td><td> ${studentData[0][i].email}</td><td> ${studentData[0][i].gender}</td><td> ${studentData[0][i].english}</td><td> ${studentData[0][i].maths}</td><td> ${studentData[0][i].science}</td><td> ${studentData[0][i].total}</td></tr>`)
...
function createSortedTableRow(arr, index1, index2){
    $('#userDataTable').empty();
        for(let i = index1; i < index2; i++){
            $('#userDataTable').append(`<tr><td> ${arr[i].id}</td><td> ${arr[i].first_name}</td><td> ${arr[i].last_name}</td><td> ${arr[i].email}</td><td> ${arr[i].gender}</td><td> ${arr[i].total}</td><td> ${arr[i].maths}</td><td> ${arr[i].science}</td><td> ${arr[i].total}</td></tr>`)
        }
}
